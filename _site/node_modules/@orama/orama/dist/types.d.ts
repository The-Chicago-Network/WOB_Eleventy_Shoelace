...
export interface IIndex<I extends AnyIndexStore> {
    create<T extends AnyOrama>(orama: T, sharedInternalDocumentStore: T['internalDocumentIDStore'], schema: T['schema']): SyncOrAsyncValue<I>;
    ...
    insertDocumentScoreParameters(index: I, prop: string, id: DocumentID, tokens: string[], docsCount: number): SyncOrAsyncValue;
    ...
    calculateResultScores<T extends AnyOrama, ResultDocument = TypedDocument<T>>(context: SearchContext<T, ResultDocument>, index: I, prop: string, term: string, ids: DocumentID[]): SyncOrAsyncValue<TokenScore[]>;
    search<T extends AnyOrama, ResultDocument = TypedDocument<T>>(context: SearchContext<T, ResultDocument>, index: I, prop: string, term: string): SyncOrAsyncValue<TokenScore[]>;
    searchByWhereClause<T extends AnyOrama, ResultDocument = TypedDocument<T>>(context: SearchContext<T, ResultDocument>, index: I, filters: Partial<WhereCondition<T['schema']>>): SyncOrAsyncValue<InternalDocumentID[]>;
...
export interface ISorter<So extends AnySorterStore> {
    create<T extends AnyOrama>(orama: T, sharedInternalDocumentStore: InternalDocumentIDStore, schema: T['schema'], sorterConfig?: SorterConfig): SyncOrAsyncValue<So>;
    insert: <T extends So>(sorter: T, prop: string, id: DocumentID, value: SortValue, schemaType: SortType, language: string | undefined) => SyncOrAsyncValue;
    ...
    sortBy<T extends AnyOrama>(sorter: So, docIds: [DocumentID, number][], by: SorterParams<T>): Promise<[DocumentID, number][]>;

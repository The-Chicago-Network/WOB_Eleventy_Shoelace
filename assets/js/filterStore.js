...
export let filterStore = {
  ...
  filterResultsByTag: function (resultsArr, tagsArr, targetProperty) {
    return resultsArr.filter((result) => {
      ...
      tagsArr.forEach((x) => {
        new RegExp(`(^${x};)|(; ${x};)|(; ${x}$)|(^${x}$)`).test(
          result.document[targetProperty],
        )
          ? null
  ...
  update: function (key, watchedElementIndex) {
    this[key] = this.watchedElements[watchedElementIndex].value
      .flat(Infinity)
      .map((x) => spacesOrUnderscores(x)); // sl-option values cannot contain spaces, so spacesOrUnderscores() will swap them to underscores
  },
  render: function (searchResults, outputContainer) {
    ...
    for (let key in filterStore) {
      if (key.match("tags")) {
        filteredResults = this.filterResultsByTag(
          filteredResults,
          filterStore[key],
          key,
        );
    ...
    resultCount.textContent =
      this.resultCount +
      `${this.resultCount != 1 ? " results found." : " result found."}`;
  ...
  watch: function (outputContainer) {
    for (let i = 0; i < this.watchedElements.length; i++) {
      this.watchedElements[i].addEventListener("sl-change", (event) => {
        ...
        this.update(key, i);
        this.isEmpty && !this.searchResults
          ? null
          : this.render(this.searchResults, outputContainer);
      });
    }
  },
};
